...
const forecast = (latitude, longitude, callback) => {
    ...
    request({url, json:true}, (error, {body}) => {
        if (error) {
            ...
            callback(undefined, 'Current temperature is ' + body.current.temperature + '. Today, sky is ' + body.current.weather_descriptions[0] + '. Wind speed today is ' + body.current.wind_speed + 'mph.')
        }
    })
